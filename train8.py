...
# Model / training config (optimized for ~17k words / ~20-25k tokens)
block_size = 128           # context length
n_embd = 256               # embedding size
n_head = 8                 # attention heads
n_layer = 4                # transformer layers
dropout = 0.45             # Balanced dropout for small dataset
batch_size = 8             # micro-batch size
grad_accum_steps = 16      # Effective batch = 8*16 = 128
max_iters = 1200           # More iterations for better convergence
...
warmup_iters = 120         # Warmup phase
use_amp = True             # Mixed precision training
use_checkpointing = True   # Gradient checkpointing to save memory
label_smoothing = 0.08     # Label smoothing for generalization
patience = 6               # Early stopping patience (increased)
seed = 42
...
# torch.compile check (skip on AMD)
_use_compile = False
try:
    is_amd = False
    ...
    if not is_amd and hasattr(torch, "compile"):
        ...
        print("AMD/ROCm detected — skipping torch.compile()")
except Exception as e:
