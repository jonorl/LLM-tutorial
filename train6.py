...
SAVE_DIR = "models"
LOG_DIR = "runs/small_gpt_run_6" # TensorBoard log directory
...
dropout = 0.40             # Increased dropout slightly from 0.35 (train5) for stronger regularization
batch_size = 8             # micro-batch on GPU
grad_accum_steps = 16      # Increased from 8 (train5) -> Effective Batch Size = 8 * 16 = 128 (Better for stability/gradient flow)
max_iters = 1000           # Increased max iters from 800 (train5) for better convergence
eval_interval = 50
eval_iters = 50
learning_rate = 3e-4       # (from train5.py)
weight_decay = 0.01        # (from train5.py)
warmup_iters = 100         # Increased warmup slightly (from 80)
use_amp = True             # mixed precision
use_checkpointing = True   # memory saving (from train5.py)
label_smoothing = 0.07     # Increased label smoothing from 0.05 (train5) for stronger generalization
